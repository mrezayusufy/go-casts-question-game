...
func (r *UserRepository) FindByPhoneNumber(phonenumber string) (*entity.User, error) {
	// query
	query := `SELECT id, name, password, phone_number FROM users WHERE phone_number = ?;`
	user := &entity.User{}
	err := r.db.QueryRow(query, phonenumber).Scan(&user.ID, &user.Name, &user.Password, &user.PhoneNumber)
	if err != nil {
		return nil, err
	}
	return user, nil
...
func (r *UserRepository) FindByID(userID uint) (*entity.User, error) {
	// query
	query := `SELECT id, name, phone_number, password FROM users WHERE id = ?;`
	user := &entity.User{}
	err := r.db.QueryRow(query, userID).Scan(&user.ID, &user.Name, &user.PhoneNumber, &user.Password)
	if err != nil {
		return nil, err
	}
	return user, nil
